...
namespace {
static std::map<sbndaq::detail::FragmentType, std::string> const names{
...
sbndaq::FragmentType sbndaq::toFragmentType(std::string t_string) {
  std::transform(t_string.begin(), t_string.end(), t_string.begin(), toupper);
  for (auto it = names.begin(); it != names.end(); ++it)
    if (t_string == it->second) return static_cast<FragmentType>(it->first);
  return FragmentType::INVALID;
}

std::string sbndaq::fragmentTypeToString(FragmentType val) {
